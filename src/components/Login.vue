    ...
    switch(this.loginType){
      case 0:
        this.eosScatter.contract(OFFICIALACCOUNTNAME).then(contract => {
          ...
          contract.addlink({player:id,upplayer:uid}, {authorization: [id]}).then(result => {
            console.log(result)
...
export default {
  ...
  computed:{
    ...
    cpuPct: function(){
      if(!this.cpu_limit||this.cpu_limit.max<0)
        return 0
      // console.log("cpu pct: "+parseInt(this.cpu_limit.used/this.cpu_limit.max*100))
      // console.log(this.cpu_limit)
      return Math.min(parseInt(this.cpu_limit.used/this.cpu_limit.max*100), 100)
    },
    netPct: function(){
      if(!this.net_limit||this.net_limit.max<0)
        return 0
      return Math.min(parseInt(this.net_limit.used/this.net_limit.max*100), 100)
    },
  },
  methods:{
    pctColor(pct){
    ...
    getAccount(){
      if(!this.isLogin)
        return
      eos.getAccount(this.eosAccountName).then((account) => {
        //console.log(account)
        this.cpu_limit = account.cpu_limit
        this.net_limit = account.net_limit
        this.ramPct = account.ram_quota <= 0 ? 0 : parseInt(account.ram_usage/account.ram_quota*100)
      }).catch((err) => {
        console.log(err)
      })
    },
    getCurrencyBalance(){
      if(!this.isLogin)
        return
      eos.getCurrencyBalance("eosio.token", this.eosAccountName).then((balance) => {
        //console.log(balance)
    ...
    chkLink(){
      ...
      if(!this.$route.params.player)
        return
      // chk accountname
      if(!Comm.IsAccountName(this.$route.params.player)){
        // myself error msg, code must be 9999
        bus.$emit("eventLink",{code:9999,msg:"errorMyLinkAccountInvalid"})
        return
      }
      // chk already linked
      eos.getTableRows(true, "eosshishicai", "eosshishicai", "link","id",this.eosAccountName, Comm.NextAccountName(this.eosAccountName)).then((result) => {
        //console.log(result)
        // must check username is same
        if(result.rows.length>0 && result.rows[0].id == this.eosAccountName){
          // already linked
          console.log("already linked to: "+result.rows[0].uid)
          this.$router.push("/")
        }else{
          // start link
          eosw.addlink(this.eosAccountName, this.$route.params.player)
        }
    ...
    initLogin(){
      ...
      if(eosw.loginType == 0 ){
        ...
        // key login
        this.eosAccountName = sessionStorage.getItem("accountName")
        this.eosAccounts = JSON.parse(sessionStorage.getItem("eosAccounts"))
        eosw.eosKey = Eos({httpEndpoint: EOSNETWORK.protocol+"://"+EOSNETWORK.host+":"+EOSNETWORK.port,chainId:EOSNETWORK.chainId, keyProvider:sessionStorage.getItem("priKey")})
        eosw.keyOptions = {
