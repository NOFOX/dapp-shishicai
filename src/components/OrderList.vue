          ...
          <table class="result-datas result-datas-eos" cellspacing="0" cellpadding="0">
            ...
            <tbody>
              <tr v-for="order in betOrders">
                <td v-text="order.num"></td>
                <td><span v-text="order.qh"></span><a v-if="order.qh != ''" :href="'https://eosflare.io/tx/'+order.id" target="_blank"><i class="fas fa-search"></i></a></td>
                <td v-if="order.type"><span class="winInfo">{{order.result}}</span>{{" - "+order.betInfo}}</td>
                <td v-else v-text="order.betInfo"></td>
                <td :class="order.type ? 'win' : 'bet'" v-text="order.betType"></td>
                <td class="td-amount" :class="{win:order.type}" v-text="order.amount.substr(0, order.amount.length-4)"></td>
                <td>{{order.time | blocktime2localtime}}</td>
              </tr>
            </tbody>
          </table>
        </div>
...
export default {
  ...
  methods: {
    ...
    tranlateBetInfo(betInfo){
      var betInfos = betInfo.split(',')
      for(var i=0;i<betInfos.length;i++){
        switch(betInfos[i]){
          case "b":
            betInfos[i] = this.$t("bsoeBig")
            break
          case "s":
            betInfos[i] = this.$t("bsoeSmall")
            break
          case "o":
            betInfos[i] = this.$t("bsoeOdd")
            break
          case "e":
            betInfos[i] = this.$t("bsoeEven")
            break
        }
      }
    ...
    getEosActions(pos,offset){
      ...
      if(this.requestCount > 0)
        return
      this.requestCount++
      eos.getActions(this.accountName,pos,offset).then((result) => {
        //console.log(result)
        ...
        for(var action of result.actions){
          //console.log(action)
          if(action.action_trace.act.account=="eosio.token" && action.action_trace.act.name=="transfer"){
            if(action.action_trace.receipt.receiver==OFFICIALACCOUNTNAME
              && action.action_trace.act.data.from==this.accountName
              && action.action_trace.act.data.to==OFFICIALACCOUNTNAME){
              // bet
              var order = {"num":action.account_action_seq,"type":0,"id":action.action_trace.trx_id,"time":action.block_time,"memo":action.action_trace.act.data.memo,"amount":action.action_trace.act.data.quantity}
              order.betType = this.$i18n.t("bet")
              order.qh = Comm.BlockTime2Qh(order.time)
              order.betInfo = order.memo.length > 32 ? order.memo.substr(0,32)+'...' : order.memo
              order.betInfo = this.tranlateBetInfo(order.betInfo)
              this.newOrder(order)

            }else if(action.action_trace.receipt.receiver==this.accountName
              && action.action_trace.act.data.from==OFFICIALACCOUNTNAME
              && action.action_trace.act.data.to==this.accountName){
              // win
              var order = {"num":action.account_action_seq,"id":action.action_trace.trx_id,"time":action.block_time,"memo":action.action_trace.act.data.memo,"amount":action.action_trace.act.data.quantity}
              ...
              switch(infos[0]){
                  ...
                  order.betInfo = subInfos[2].length > 25 ? subInfos[2].substr(0,25)+'...' : subInfos[2]
                  order.betInfo = this.tranlateBetInfo(order.betInfo)
                  break
                case "rebate":
                  order.type = 2
                ...
                default:
                  if(infos.length == 3){
                    ...
                    order.betInfo = infos[2].length > 25 ? infos[2].substr(0,25)+'...' : infos[2]
                    order.betInfo = this.tranlateBetInfo(order.betInfo)
                  }else{
                    continue
                  }
...
.result-datas thead th.th-num{
  width:40px;
}
.result-datas thead th.th-qh{
  width:120px;
}
.result-datas thead th.th-type{
  width:50px;
}
.result-datas thead th.th-amount{
  width:120px;
}
.result-datas thead th.th-time{
  width:230px;
}
...
.result-datas-eos tbody td.td-amount {
  text-align:right;
}
