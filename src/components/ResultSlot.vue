...
export default {
  ...
  methods: {
    slotNum(num) {
      ...
      this.currentNum = num;
      if (num == "" || num == " ")
        this.$refs.num1.style.animation =
          "kfLoading 2s linear infinite " + Math.random() * 0.5 + "s";
      else
        this.$refs.num1.style.animation =
          "kfNum" + num + " 5s ease-out forwards";
    },
  ...
  mounted() {
    ...
    this.$refs.num1.style.backgroundSize =
      this.$refs.num1.clientWidth + "px " + numHeight * 10 + "px";
    this.insertStyleSheetRule(
      "@keyframes kfLoading { 0% { background-position: 0% 0%; } 100% { background-position: 0% -110%; } }"
    );
    for (var i = 0; i < 10; i++) {
      this.insertStyleSheetRule(
        "@keyframes kfNum" +
          i +
          " { 0% { background-position: 0px 0px; } 100% { background-position: 0px " +
          (20 - i) * numHeight +
          "px; } }"
      );
    }
...
<style scoped>
.slot {
  /* background: url(../assets/img/num_box.jpg); */
  background-size: 100% 100%;
  position: relative;
}
.num_mask {
  /* background: url(../assets/img/num_mask.png); */
  background-size: 100% 100%;
  width: 100%;
  height: 55%;
  z-index: 9;
...
.num {
  /* background: url(../assets/img/num.png) top center repeat-y; */
  position: absolute;
  width: 100%;
  height: 100%;
}
